...
AIRP에 관심 가져주셔서 감사합니다. 작은 수정도 환영합니다.
...
```bash
uv run airp --dry-run      # 발송 없이 콘솔에서 결과 확인
uv run pytest -q           # 테스트
...
- **새 소스 추가**: `config/sources.yaml`에 RSS 한 줄, 또는 `airp/collectors/`에 수집기
- **카탈로그 카테고리**: `airp/catalog.py`의 규칙/카테고리 개선
- **알림 채널**: `airp/notify/`에 새 채널(Discord, Telegram 등)
- **문서 · 번역**
